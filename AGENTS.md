...
- **PowerShell スクリプト**:
  ...
  - 各スクリプトは `DOTNET_EXE` が指定されていればその `dotnet.exe` を使い、未指定でも `%USERPROFILE%\.dotnet\dotnet.exe` があれば優先する。既定の `dotnet` が .NET 10 SDK を指さない環境では、`DOTNET_EXE` に使用する `dotnet.exe` のフルパスを指定する。
