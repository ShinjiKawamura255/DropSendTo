...
- `Slot Layout` メニューから行/列を 2〜8 で直接入力でき、最大 8 行×8 列（64 スロット/レイヤー）まで拡張可能。`Slot Size` は Small/Medium/Large（既定 Medium）または Custom から選択。Small=1 行、Medium=2 行、Large=3 行表示。Custom ではスロット高さ/フォントサイズ/スロット間隔（余白・行列ステップ）を任意に設定できます。
- レイアウトを大きく入れ替えたいときはツールバーの ✎ を押してスロット編集モードへ。モード中は左上に `SLOT SETUP MODE` インジケーターが出現し、スロットをドラッグ＆ドロップでスワップできます。ドラッグ中に現在のレイヤーボタンへ 0.8 秒以上ホバーするとそのレイヤーが自動的に表示され、空スロットへドロップすれば単純な移動として反映されます。入れ替え対象は元の位置にプレビュー表示され、ドロップした瞬間にのみ確定します（モード中はクリック/ファイルドロップ実行は無効化されます）。さらに、スロット編集モード中に空スロットへファイル/フォルダ/テキストをドロップすると Command モードで簡易登録できます。フォルダをドロップするとコマンド欄にそのフォルダパスが入り、既定のアプリで開けます（ファイルは拡張子なしの名前をタイトルに設定）。
- `Always on Top` のチェックを外すと通常ウィンドウ化、`Minimize to Tray` や Prefix+Shift+Enter でトレイへ格納できます。
- `Ctrl+F` で検索レイヤーを呼び出せます（Emacs ライク操作が有効なら `Ctrl+S`、vi ライク操作が有効なら `/`）。レイヤー名オーバーレイと同じ位置に検索ボックスが出現し、Title と Search Keywords から検索した結果だけを 1 番から詰めて表示します。デフォルトはあいまい検索（部分一致/サブシーケンス）ですが、**`flistwalker` 互換の検索演算子** を使用可能です。
  - `'` (シングルクォート): `'abc` のように指定すると、`abc` という文字列そのものを連続して含む「完全一致 (部分一致)」検索を行います。
  - `!` (感嘆符): `!abc` や `!'abc` のように指定すると、そのキーワードを含まないスロットを表示する「除外」検索を行います（除外は常に完全一致で行われます）。
  - `^` (ハット) / `$` (ドル記号): `^abc` は先頭一致、`abc$` は末尾一致となります（デフォルトのあいまい一致、または `'` を組み合わせた完全一致に適用されます）。
  - `|` (縦棒): `abc|def` のように指定すると、どちらかに一致するスロットを表示する「OR」検索を行います。
  - これらを組み合わせて、`^'abc` (先頭完全一致) や `!'abc$` (末尾に `abc` が完全一致で含まれないもの) のように高度な絞り込みが可能です。
  - スペースで区切ると、それらすべての条件を満たすスロットを検索する「AND」検索になります。
  - Esc で閉じられます。検索中は Tab に加えて、検索ボックスがウィンドウ上側にあるときは ↓（Emacs: `Ctrl+N`）でスロットへ移り、最上段で ↑（`Ctrl+P`）を押すと検索ボックスへ戻ります。検索ボックスが下側に回り込んだときはこの上下が逆転します。vi の `j/k` では検索ボックスとの行き来は行わず、スロット内の移動のみです。
...
2. Title / Command / Arguments / Macro Script / Shortcut を入力  
   ...
   - Search Keywords 欄にスペース区切りで検索用キーワードを入れておくと、検索レイヤーから素早く見つけられます（Title と合わせてあいまい/演算子検索可能）
...
6. 「起動時にこのスロットを実行」をオンにすると、DropSendTo 起動時に該当スロットが自動で実行されます。
...
- `起動時実行スロット...` で、起動時に実行するスロットを一覧で確認・編集できます。  
- `スタートアップに登録` をオンにすると、`shell:startup` フォルダへ手動で配置しなくても、現在のユーザーの Windows サインイン時に DropSendTo を自動起動できます。解除すると登録が削除されます。
- `Macro 実行モード` で排他／割り込み／一時停止を選ぶと、その場でマクロ挙動が変わります。  
- コンテキストメニューの `テーマ` からダーク／ライトを切り替え可能（既定はダーク）。  
- マウスでウィンドウを呼び出す場合はコンテキストメニューの「マウスジェスチャ (表示/最小化)...」から設定できます。既定は「時計回り 3 周で表示、反時計回り 2 周で最小化」。方向の入れ替え、表示ジェスチャのみ Ctrl 同時押しを必須にする設定、発表モード時の無効化に加え、ジェスチャ判定の半径もスライダーで調整できます（ドラッグ中はカーソルを中心にガイド円が表示され、半径制限のチェックを外すと制限を無効化できます）。あわせて、ドラッグ中のホイールクリックでドロップ専用ウィンドウを表示するオプション（既定 OFF）も同じダイアログから切り替え可能です。  
- ドロップ専用ウィンドウにファイル/フォルダを落とすと左上に `Dropped` インジケーターが点灯し、その状態でスロットをクリック/選択/検索で起動すると `{args}` として渡されます。  
- 設定・ログの場所  
...
1. `Change Prefix...` で Prefix（既定 `Ctrl+Q`）を設定。  
   - 不正な入力はダイアログで拒否され、保存されません。既存またはインポート済みの設定値が壊れている場合は、起動時に `Ctrl+Q` へ戻して警告します。
...
4. 組み合わせ一覧（いずれも Prefix 押下後）  
   ...
   | `Alt+Space` | 検索レイヤーを開いて DropSendTo を前面に復帰 | 検索入力へフォーカス。表示位置はメニュー「表示挙動 > 表示位置 (検索)」で固定/マウスフォロー/カーソルのある画面中央から選択可（座標は保存しない） |
...
- 引数テンプレートで使えるプレースホルダ  
  - `{args}` / `{drop_args}`: ドロップ/CLI のパス  
  - `{drop_count}`: ドロップ件数  
  - `{drop_path}` / `{drop_path:n}`: 先頭または n 番目(1〜)のパス  
...
- 任意の入力を受けたい場合は `PROMPT <変数> "メッセージ" [DEFAULT "初期値"] [TIMEOUT <ms> "タイムアウト値"]` でメッセージ付きの入力ダイアログを表示し、OK で入力値を変数へ格納します（キャンセルするとマクロを失敗として扱います。DEFAULT 省略時は空文字が入ります。TIMEOUT を指定すると指定時間経過でタイムアウト値が格納されます）。
...
より詳細なコマンド一覧はアプリ内の `?` ボタン（Macro Script ダイアログ右上）から参照できます。リポジトリ内の `docs/MACRO_SAMPLES.md` には、現行コマンドだけで動くコピー＆ペースト用サンプルもあります。

### 2.7 CLI・自動化のヒント
- `DropSendTo.exe "<path>"` で現在レイヤーの最初の登録スロットを自動実行。PowerShell などからまとめて呼び出せます。  
- 設定を別環境へ移行したい場合はコンテキストメニューから `Export/Import Config...` を利用できます。エクスポート内容は暗号化されますが、復号後の設定にはアプリ起動、マクロ、ファイル操作、起動時実行が含まれ得ます。パスワードは別チャネルで共有し、信頼できる入手元のファイルだけをインポートしてください。インポート時は実行可能設定の件数を確認し、内容を理解した場合だけ確認チェックを付けてください。受理した設定は直ちに画面・ショートカット・ジェスチャ等へ反映され、適用失敗時は以前の設定へ戻ります。
