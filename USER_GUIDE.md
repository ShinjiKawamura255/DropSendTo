...
- `Slot Layout` メニューで最大 8 行×4 列（32 スロット/レイヤー）、`Slot Size` で Small/Medium/Large（既定 Medium）または Custom を選択。Small=1 行、Medium=2 行、Large=3 行表示。Custom ではスロット高さ/フォントサイズ/スロット間隔（余白・行列ステップ）を任意に設定できます。
