...
- StartupRegistrationService: 現在ユーザーの `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` に `DropSendTo` 値を作成/削除し、起動コマンドを引用符付きで保存する。配布 exe を直接起動するほか、開発時の `dotnet` ホストではエントリ DLL を引数に含める。登録状態の読み取りは UI メニューのチェック状態同期に利用し、設定 JSON には複製しない。
...
- ConfigTransferService / ConfigImportCoordinator: `AppConfig`、`Layer`、`SlotModel`、`SlotMinimizeOptions` の明示 snapshot を AES-GCM + PBKDF2 で転送し、復号前に package/KDF/暗号要素の上限を検証する。全 writable property の非既定値 round-trip guard が snapshot 宣言だけでなく双方向写像を検証する。`ConfigImportRiskSummary` が実行可能設定を集計し、モデルレス確認後だけ coordinator が候補を runtime へ段階適用してから保存する。runtime 適用失敗時は disk を変更せず旧 runtime を復元し、保存失敗時は旧 disk/runtime を個別に復元する。未復元箇所は構造化してユーザーへ明示する。
...
- LayerButtonModelFactory: 総レイヤー数、現在レイヤー、ボタン数、直前の移動方向を明示入力として、数値ボタン・前後矢印・非表示ボタンの表示モデルを返す純粋サービス。MainWindow は WPF ボタンへの反映だけを担う。
...
6) メニュー操作: メニューボタン/ウィンドウ右クリックで Open Config/Open Logs/Change Prefix/Slot Layout/Language/起動時のウィンドウ/スタートアップ登録/常に最前面トグル/Exit を提供。Open Config/Logs は `Process.Start` with `UseShellExecute=true`。常に最前面トグルは Topmost と config を即時更新し、Language メニューでは Japanese/English を選択するとメニューと検索ラベルの文言を即時切り替えた上で `_config.Language` とメニューのチェック状態を保存する。起動時のウィンドウは「常に表示」「前回の状態を復元」「常にタスクトレイで起動」の 3 つから選択し、設定を即時保存する。スタートアップ登録は `StartupRegistrationService` を介して HKCU の Run 値を即時更新し、メニューを開くたびに実際の登録状態をチェックへ反映する。
...
- StartupRegistrationService
  - `IsRegistered()`: 現在ユーザーの Windows Run キーに `DropSendTo` 値が存在するか返す。
  - `Register()` / `Unregister()`: `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` の値を現在の起動コマンドで設定/削除する。管理者権限を要求せず、設定 JSON には保存しない。
