...
- TC-034 MacroPrompt: `PROMPT <変数> "メッセージ" [DEFAULT "初期値"]` でメッセージと初期値が表示され、OK を押すと入力値が変数へ格納され `RETURN "{{変数}}"` 等で参照できる。DEFAULT 省略時は空文字が入ること、キャンセル時はマクロが失敗することを確認する。
...
- TC-061 WindowPosTransient: ドラッグ中のホイールクリックでウィンドウを表示しても、固定位置の保存値が更新されないことを確認する。
...
- FR-023 → SP-010 → DES-002/003 → TC-085/086/088/089/099/100/101
