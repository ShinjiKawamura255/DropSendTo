...
## SP-001 UI/Window
- MUST: スロットグリッドは設定で決まる行列（2〜8 行 × 2〜8 列）を使用し、既定値は 2x2。設定された 4〜8 レイヤーを切り替えて表示する。
...
- MUST: レイヤー 1〜設定数（4〜8）を UI ボタンで切替。ボタンは現在レイヤーを強調表示。
...
- MUST: 設定保存は primary と同じディレクトリへ一時ファイルを書き、flush 後に原子的な置換または移動で確定する。書き込み、flush、置換のいずれかが失敗した場合も、既存の正常な primary と backup を維持し、残留一時ファイルを次回保存前に除去する。
...
- MUST: `WIFI_SSID <変数>` は接続中の Wi-Fi SSID を変数へ格納する。未接続の場合は空文字列を格納する。SSID 取得は Windows WLAN API を優先し、OS の表示言語や `netsh` 出力形式に依存してはならない。WLAN API が利用できない場合のみ `netsh wlan show interfaces` をフォールバックとして使用してよい。
...
## SP-015 Operational Privacy
- MUST: 通常ログには展開済み引数、コマンドパス、スロットタイトル、クリップボード/ドロップ値、マクロ変数値、`RETURN` 値を記録せず、件数、長さ、状態、エラー種別などの診断メタデータを記録する。
- MUST: 起動・マクロ・UI 統合経路は同一のロガー契約を使用し、テストでは sentinel 値が通常ログへ現れないことを検証できる。
...
- MUST: 検索レイヤーのスロット検索対象は `Title` と `SearchKeywords` のみとする。Command、ArgumentsTemplate、KeyboardMacroScript は検索対象に含めない。空クエリでは全レイヤーの非空スロットをレイヤー順・スロット順で表示候補にする。非空クエリは空白区切り token の AND 条件とする。各 token のデフォルトはあいまい検索（部分一致または subsequence 照合）とし、大文字小文字・全角/半角・濁点の正規化、検索語側のハイフン/長音記号除去、かな→ローマ字候補（二合仮名や歴史的仮名遣い含む）で評価する。演算子として、完全一致（`'` または `^'`）、除外（`!`）、先頭一致（`^`）、末尾一致（`$`）、OR 検索（`|`）をサポートし、`!` は常に完全一致で除外する。
