...
## SP-001 UI/Window
- MUST: スロットグリッドは設定で決まる行列（2〜8 行 × 2〜4 列）を使用し、既定値は 2x2。常に 4 レイヤーを切り替えて表示する。
...
- MUST: Slot Size メニューでは Small/Medium/Large を提供し、タイトル表示行数をそれぞれ 1 行・2 行・3 行に制限する（Small はステータスを同一行に重ねる）。選択時はスロット高さ/フォントサイズ/ウィンドウ幅を再計算して即時反映し、設定にも保存する。既定値は Medium。
...
- MUST: Slot Layout サブメニューは 2〜8 行×2〜4 列の組み合わせを表示し、選択と同時に UI を更新して設定へ保存する。
...
- MUST: スロットショートカットは 1 個以上の KeyChord を空白/カンマ/`>` 区切りで入力でき、Prefix 後は 1 個目の一致で入力を抑止する。同じ開始キーで複数候補がある場合は次のキーを待機し、途中で Tab/Enter/Prefix+Ctrl+N/P などの特殊操作や別キーが入力されたら待機を解除して特殊操作を優先する。
- MUST: RDP/Citrix のクライアントウィンドウが前面にある間はローカル DropSendTo の Prefix/ショートカット処理を停止し、リモート側を優先する設定を提供する（既定で有効）。クライアント以外へフォーカスが戻った場合はローカル DropSendTo へ自動復帰する。
- MUST: Prefix と同じ修飾キーを含むショートカットは、修飾キーを押し直さずに Prefix 入力直後から検出でき、必要に応じて再押下しても正常に動作する。
- MUST: コンテキストメニューの「Prefix: Ctrl+N/P でレイヤー切替」を有効にしている場合、Prefix armed 中に `Ctrl+N` で次レイヤー、`Ctrl+P` で前レイヤーへ循環切替する。既定は無効で、トグル状態は config.json に保存・復元する。
...
- スロット数は各レイヤーで SlotRows×SlotColumns（2〜8 行×2〜4 列）に一致し、空スロットは「未登録」表示。
...
## SP-012 Slot Layout Edit Mode
- MUST: ツールバーの ✎ ボタンおよびウィンドウのコンテキストメニューから Slot Layout Edit Mode をトグルでき、モード中は左上に「EDIT MODE」インジケーターを表示し、スロットクリック/ファイルドロップ起動を抑止する。
...
- MUST: ドラッグ中にレイヤーボタンを ≥0.8 秒ホバーすると表示レイヤーを切り替え、別レイヤーのスロットとも同一操作でスワップできる。ドラッグキャンセルやモード解除時はプレビューを即座に消し、インジケーターを「EDIT MODE」に戻す。
