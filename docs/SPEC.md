...
- MUST: `PROMPT <名前> "メッセージ" [DEFAULT "初期値"]` はメッセージ付きの入力ダイアログを表示し、OK 時に入力した文字列を変数 `<名前>` へ格納する。メッセージと初期値は変数展開可能とし、初期値を省略した場合は空文字列を格納する。検証モードではダイアログを表示せず初期値を設定し、キャンセルした場合はマクロ失敗として復帰する。
