...
- SHOULD: 最終選択レイヤーを記憶。
- MUST: ドラッグ中、レイヤーボタン上へ ≥1.0s 滞在でそのレイヤーに自動切替（通常時は従来どおりクリック動作）。
...
- MUST: メニューに「設定ファイルを開く」を提供（既定アプリで `config.json` を開く）。
